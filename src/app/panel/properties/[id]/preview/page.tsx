...
export function generateStaticParams() {
  return panelProperties.map((item) => ({ id: item.listing.id }));
}
...
export default async function PropertyPreviewPage({
  ...
  const item = getPanelProperty((await params).id);
  if (!item) notFound();

  const listing = item.listing;

  const facts: [string, string][] = [
    ["متراژ", `${listing.area} متر`],
    ["تعداد خواب", `${listing.rooms} خواب`],
    ["قیمت", listing.price],
  ];
  ...
  return (
    <div>
      <PanelPageHeader
        ...
        description="نمای تقریبی فایل پیش از انتشار عمومی."
        action={
          <Button
            variant="outline"
            nativeButton={false}
            render={<Link href={routes.panel.adManagement(listing.id)} />}
          >
            <ArrowRight />
            بازگشت به مدیریت
          </Button>
          ...
          <CardContent className="p-5">
            <div className="flex items-center gap-2">
              ...
              <Typography variant="small">
                این صفحه برای کاربران عمومی قابل مشاهده نیست.
              </Typography>
            </div>
            ...
            <div className="mt-6 rounded-xl border border-dashed p-5">
              ...
              <Typography variant="muted" className="mt-2 leading-7">
                عنوان، قیمت، محله و مشخصات اصلی را بررسی کنید. تصاویر و اطلاعات
                تماس پس از اتصال به سرویس بک‌اند از پرونده ملک دریافت می‌شوند.
              </Typography>
            </div>
          </CardContent>
        </Card>
