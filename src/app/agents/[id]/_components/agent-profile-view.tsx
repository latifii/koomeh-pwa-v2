...
import { AgentRow } from "@/app/agents/_components/agent-card";
    ...
    <div className="pb-16">
      ...
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "مشاورین", path: routes.agents },
      ...
      <Breadcrumb
        items={[
          ...
          { label: "مشاورین", href: routes.agents },
          { label: agent.name },
        ]}
      />
      ...
      <Container className="mt-5">
        <div className="grid items-start gap-5 lg:grid-cols-3">
          <main className="grid min-w-0 gap-5 lg:col-span-2">
            {agent.bio && (
              <Section icon={UserRound} title={`درباره‌ی ${agent.name}`}>
          ...
          <aside className="grid gap-4 lg:sticky lg:top-20">
            ...
            {otherAgents.length > 0 && (
              <Section icon={Award} title="مشاورین دیگر" padded={false}>
                <div className="grid gap-1 p-2">
                  {otherAgents.map((item) => (
                    <AgentRow key={item.id} agent={item} />
                  ))}
                </div>
              </Section>
            )}
...
function Expertise({ agent }: { agent: AgentDto }) {
  if (
    !agent.estate_types.length &&
    !agent.districts.length &&
    !agent.languages.length
  ) {
    return null;
  }
  return (
    <Section icon={Building2} title="حوزه‌ی فعالیت">
      <div className="grid gap-4 sm:grid-cols-2">
        {agent.estate_types.length > 0 && (
          <TagGroup
            title="نوع ملک"
            icon={Building2}
            items={agent.estate_types.map((item) => item.label)}
          />
        )}
        {agent.districts.length > 0 && (
          <TagGroup
            title="محله‌های فعالیت"
            icon={MapPin}
            items={agent.districts}
          />
        )}
        {agent.languages.length > 0 && (
          <TagGroup
            title="زبان‌های ارتباطی"
            icon={Languages}
      ...
      <div className="flex flex-wrap gap-1.5">
        {items.map((item) => (
          <Typography
            ...
            variant="small"
            key={item}
            className="flex items-center gap-1.5 rounded-lg bg-muted/60 px-2.5 py-1"
          >
...
function Listings({
  ...
  agent: AgentDto;
  counts: { all: number; sale: number; rent: number };
  listings: Listing[];
}) {
  return (
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <Typography variant="small" className="mt-0.5">
            {counts.all.toLocaleString("fa-IR")} فایل در دست {agent.name}
          </Typography>
        </div>
        <div className="flex gap-1.5">
          ...
          >
            فروش {counts.sale.toLocaleString("fa-IR")}
          </Typography>
          <Typography
            as="span"
            variant="small"
            className="rounded-full bg-secondary/15 px-2.5 py-1 text-foreground"
          >
            اجاره {counts.rent.toLocaleString("fa-IR")}
          </Typography>
        </div>
      </div>
      {listings.length ? (
        <div className="-mx-page flex gap-3 overflow-x-auto overflow-y-hidden px-page pb-2 sm:mx-0 sm:grid sm:grid-cols-2 sm:px-0">
          {listings.map((listing) => (
            <PropertyCard
              key={listing.id}
              estate={listing}
              className="w-[70vw] shrink-0 sm:w-auto"
            />
          ))}
        </div>
      ) : (
        <EmptyState
          icon={Home}
          title="در حال حاضر فایل فعالی ندارد"
          description="برای دریافت فایل‌های جدید با این مشاور در تماس باشید."
        />
      )}
    </section>
  );
